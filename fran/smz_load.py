...
def generate_spike_counts(recording_name, brain_region, minimum_grade, dt, t0s, duration, firing_rates=False):
    ...
    lengths = []
    for i_trial, t0 in enumerate(t0s):

        spike_counts = np.zeros((n_time_bins, n_neurons))
        for i_neuron, neuron_id in enumerate(neuron_indices):

            # for each neuron, retrieve its spikes
            neuron_spikes = spikes[spikes_2_neurons == neuron_id]

            # generate an histogram of spike counts
            [neuron_spike_counts, _] = np.histogram(neuron_spikes, t0 + time_steps)
            spike_counts[:, i_neuron] = neuron_spike_counts

        if i_trial == 0:
            spike_counts_sequence = spike_counts
        else:
            spike_counts_sequence = np.concatenate([spike_counts_sequence, spike_counts])
        lengths.append(len(spike_counts))

    return spike_counts_sequence, lengths, time_bins
