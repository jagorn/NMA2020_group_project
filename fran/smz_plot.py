...
def plot_psths(dataset, time_bins, title, visual_time, cue_time, feedback_time):
    ...
    # x_ticks = np.arange(len(time_bins))
    # x_labels = (time_bins[x_ticks] - time_bins[0]) * 1000
    # plt.xticks(x_ticks, x_labels.astype(int).astype(str))

    # visual_time_bin = np.where(visual_time < time_bins)[0][0]
    # plt.axvline(visual_time_bin, color='r')

    cue_time_bin = np.where(cue_time < time_bins + 0.0005)[0][0]
...
def add_states_2_psth(fig, states, colors, n_neurons):
    ...
    for state_change in state_changes:
        ...
        rect = patches.Rectangle((previous_state_change-1, 0), state_dt, n_neurons, edgecolor='none', facecolor=c, alpha=0.2)
        fig.add_patch(rect)
